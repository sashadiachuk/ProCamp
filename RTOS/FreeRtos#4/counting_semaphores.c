...
//configUSE_COUNTING_SEMAPHORES 1 seted in freertos.h
...
void TaskBlinkRED(void *pvParameters)//MY
{
	 ...
	 xSemaphoreGive(CountingSem);//Release two Semaphore
	 xSemaphoreGive(CountingSem);
	 for (;;) {
		xSemaphoreTake(CountingSem, portMAX_DELAY);
...
int main(void)
{
  ...
  /* add threads, ... */
  CountingSem = xSemaphoreCreateCounting(2,0);//Semaphore will count until 2
   if(CountingSem != NULL)
   {
